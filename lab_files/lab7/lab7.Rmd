...
Najbardziej istotne są te, które znajdują się wysoko w drzewie, ponieważ najskuteczniej je dzielą. Są to m.in. ShelveLoc, Price, Income.
...
```{r}
accs = c()
for (s in head(sales_high_cv$size, -1)) {
  sales_high_pruned <- prune.misclass(sales_high_tree, best = s)
  pruned_class <- predict(sales_high_pruned, newdata = CarseatsH[test,], type = "class")
  acc <- mean(pruned_class == CarseatsH$High[test])
  accs = c(accs, acc)
}
plot(head(sales_high_cv$size, -1), accs, type = "b")
```
...
Najistotniejsze są zdecydowanie liczba pokoi i status społeczny, wyróżniają one kilka różnych typów miejsc zamieszkania (bogate śródmieście, biedniejsze przedmieścia)
...
```{r}
mses = c()
for (s in head(medv_cv$size, -1)) {
  medv_pruned <- prune.tree(medv_tree, best = s)
  medv_pred <- predict(medv_pruned, newdata = Boston[test,])
  mse <- mean((medv_pred - Boston$medv[test])^2)
  mses <- c(mses, mse)
}
size_opt <- head(medv_cv$size, -1)[which.min(mses)]
cat("Optimal size:", size_opt, "\nMSE:", min(mses))
```
...
predyktorów?**
```{r}
medv_bag <- randomForest(medv ~ ., data = Boston, mtry = 13, importance = TRUE, subset = train)
importance(medv_bag)
varImpPlot(medv_bag)
```
Wygląda na to, że się zmniejszyły
...
varImpPlot(medv_rf)
```
Kolejność jest taka sama, ale w IncNodePurity rm i lstat są większe niż w baggingu.

**7. Porównaj na wykresie błędy OOB dla baggingu i domyślnie skonfigurowanego lasu losowego.**
```{r}
plot(1:medv_rf$ntree, medv_rf$mse, type="l", col="red", xlab="trees", ylab="mse")
lines(1:medv_bag$ntree, medv_bag$mse, type="l", col="green")
```
Błąd dla baggingu większy
